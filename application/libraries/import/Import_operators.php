...
class Import_operators extends App_import
{
    ...
    public function __construct()
    {
        ...
        $this->addImportGuidelinesInfo('Make sure you configure the default contact permission in <a href="' . admin_url('settings?group=operators') . '" target="_blank">Setup->Settings->Operators</a> to get the best results like auto assigning contact permissions and email notification settings based on the permission.');
        ...
        foreach ($this->getRows() as $rowNumber => $row) {
            ...
            for ($i = 0; $i < $totalDatabaseFields; $i++) {
                ...
                if (in_array($databaseFields[$i], $this->requiredFields) &&
                    ...
                    if (is_automatic_calling_codes_enabled() && !empty($row[$i])) {
                        $operatorCountryIndex = array_search('country', $databaseFields);
                        if (isset($row[$operatorCountryIndex]) && !empty($row[$operatorCountryIndex])) {
                            $operatorCountry = $this->getCountry(null, $this->countryValue($row[$operatorCountryIndex]));

                            if ($operatorCountry) {
                                $callingCode = '+' . ltrim($operatorCountry->calling_code, '+');

                                if (startsWith($row[$i], $operatorCountry->calling_code)) { // with calling code but without the + prefix
                                    $row[$i] = '+' . $row[$i];
            ...
            if (count($insert) > 0) {
                ...
                if (!$this->isSimulation()) {
                    ...
                    if ($this->shouldAddContactUnderOperator($insert)) {
                        $this->addContactUnderOperator($insert);

                        continue;
                    }

                    $insert['is_primary'] = 1;
                    $id                   = $this->ci->operators_model->add($insert, true);

                    if ($id) {
                        if ($this->ci->input->post('groups_in[]')) {
                            $this->insertOperatorGroups($this->ci->input->post('groups_in[]'), $id);
                        }

                        if (staff_cant('view', 'operators')) {
                            $assign['operator_admins']   = [];
                            $assign['operator_admins'][] = get_staff_user_id();
                            $this->ci->operators_model->assign_admins($assign, $id);
                        }
                    }
                } else {
                ...
                $this->handleCustomFieldsInsert($id, $row, $i, $rowNumber, 'operators');
            }
    ...
    {
        return admin_url('operators/import');
    }
    ...
    private function insertOperatorGroups($groups, $operator_id)
    {
        foreach ($groups as $group) {
            $this->ci->db->insert(db_prefix() . 'operator_groups', [
    ...
    private function shouldAddContactUnderOperator($data)
    {
        return (isset($data['company']) && $data['company'] != '' && $data['company'] != '/')
        && (total_rows(db_prefix() . 'operators', ['company' => $data['company']]) === 1);
    }

    private function addContactUnderOperator($data)
    {
        $contactFields = $this->getContactFields();
        $this->ci->db->where('company', $data['company']);

        $existingCompany = $this->ci->db->get(db_prefix() . 'operators')->row();
        $tmpInsert       = [];
        ...
        $this->ci->operators_model->add_contact($tmpInsert, $existingCompany->userid, true);
    }
