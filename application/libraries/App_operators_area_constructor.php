...
class App_operators_area_constructor
{
    private $ci;
    ...
    public function __construct()
    {
        ...
        $vars = [];
        if (is_operator_logged_in()) {
            $contact            = $this->ci->operators_model->get_contact(get_contact_user_id());
            ...
            $vars['operator']                          = $this->ci->operators_model->get($contact->userid);
            $vars['contact']                         = $contact;
        }

        init_operators_area_assets();

        hooks()->do_action('operators_init');
        ...
        $vars = hooks()->apply_filters('operators_area_autoloaded_vars', $vars);
