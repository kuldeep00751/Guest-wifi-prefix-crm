...
define('OPERATORS_AREA', true);

class OperatorsController extends App_Controller
{
    public $template = [];
    ...
    public function __construct()
    {
        ...
        $this->load->library('app_operators_area_constructor');
    ...
    public function layout($notInThemeViewFiles = false)
    {
         ...
         * @var string
         */
        $this->template['head'] = $this->load->view('themes/' . active_operators_theme() . '/head', $this->data, true);
        ...
        ? $this->load->view('themes/' . active_operators_theme() . '/footer', $this->data, true)
        : '';
        ...
        if (file_exists(VIEWPATH . 'themes/' . active_operators_theme() . '/scripts.php')) {
            if (ENVIRONMENT != 'production') {
                trigger_error(sprintf('%1$s', 'Operators area theme file scripts.php file is no longer used since version 2.3.0, add app_customers_footer() in themes/[theme]/index.php. You can check the original theme index.php for example.'));
            }

            $this->template['scripts'] = $this->load->view('themes/' . active_operators_theme() . '/scripts', $this->data, true);
        }

        /**
         * Load the theme compiled template
         */
        $this->load->view('themes/' . active_operators_theme() . '/index', $this->template);
    }

    /**
     * Sets view data
     * @param  array $data
     * @return core/OperatorsController
     */
    ...
    */
    ...
    {
        return 'themes/' . active_operators_theme() . '/views/' . $view;
    }
}
