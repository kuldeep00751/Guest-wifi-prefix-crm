...
<div class="form-group">
	<label for="operators_default_theme" class="control-label"><?php echo _l('settings_operators_default_theme'); ?></label>
	<select name="settings[operators_default_theme]" id="operators_default_theme" class="form-control selectpicker" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
		<?php foreach (get_all_operator_themes() as $theme) { ?>
		<option value="<?php echo e($theme); ?>" <?php if (active_operators_theme() == $theme) {
    echo 'selected';
...
?>
<div class="form-group">
	<label for="visible_customer_profile_tabs" class="control-label"><?php echo _l('visible_tabs'); ?> (<?php echo _l('operator_add_edit_profile'); ?>)</label>
...
<div class="form-group">
	...
	<select name="settings[required_register_fields][]" id="required_register_fields" multiple class="form-control selectpicker" data-none-selected-text="<?php echo _l('all'); ?>" data-actions-box="true">
		...
		<?php foreach($optionFields['company'] as $key => $field) { ?>
			<option value="<?php echo $key; ?>" <?php echo $field['is_required'] ? 'selected' : '';?> <?php echo $field['disabled'] ? 'disabled' : ''; ?>>
			<?php echo $field['label'] . ' - ' . _l('operator_company'); ?>
		</option>
		<?php } ?> 
	</select>
</div>
...
<hr />
<?php render_yes_no_option('allow_registration', 'settings_operators_allow_registration'); ?>
<hr />
...
<hr />
<?php render_yes_no_option('knowledge_base_without_registration', 'settings_operators_allow_kb_view_without_registration'); ?>
<hr />
<div class="form-group">
	<?php
...
if (count($custom_fields) > 0) {
    ...
    if (total_rows(db_prefix() . 'customfields', ['fieldto' => 'customers', 'show_on_operator_portal' => 1]) == 0) {
