...
<?php $tabs = filter_operator_visible_tabs($operator_tabs, $operator->userid); ?>
...
<nav class="operator-tabs tw-hidden tw-flex-1 tw-flex-col md:tw-flex" aria-label="Sidebar">
  <ul role="list" class="tw-space-y-0.5">
    <?php foreach ($tabs as $key => $tab) { ?>
    <?php $current = (! $this->input->get('group') && $key === 'profile') || $this->input->get('group') === $key; ?>
    <li class="operator_tab_<?= e($key); ?>">
      <a href="<?= admin_url('operators/operator/' . $operator->userid . '?group=' . $key); ?>"
        data-group="<?= e($key); ?>"
...
<script>
  function redirectToTab(selectElement) {
    ...
    if (key) {
      const operatorId = <?= json_encode($operator->userid); ?> ;
      window.location.href =
        `<?= admin_url('operators/operator/'); ?>${operatorId}?group=${key}`;
    }
  }
</script>