...
<?php
/**
 * Included in application/views/admin/operators/operator.php
 */
?>
<script>
Dropzone.options.operatorAttachmentsUpload = false;
var operator_id = $('input[name="userid"]').val();
$(function() {

    if ($('#operator-attachments-upload').length > 0) {
        new Dropzone('#operator-attachments-upload', appCreateDropzoneOptions({
            paramName: "file",
    ...
    if(tab_active === 'operator_admins') {
        $('#profile-save-section').addClass('hide');
    }

    $('a[href="#operator_admins"]').on('click', function() {
        $('#profile-save-section').addClass('hide');
    });

    $('.operator-profile-tabs  a').not('a[href="#operator_admins"]').on('click', function() {
    ...
    if (contact_id) {
        contact(operator_id, contact_id);
    }
    ...
    $('body').on('change', '.onoffswitch input.operator_file', function(event, state) {
        var invoker = $(this);
        var checked_visibility = invoker.prop('checked');
        var share_file_modal = $('#operator_file_share_file_with');
        setTimeout(function() {
    ...
    $('.operator-form-submiter').on('click', function() {
        var form = $('.operator-form');
        if (form.valid()) {
    ...
    if (typeof(Dropbox) != 'undefined' && $('#dropbox-chooser').length > 0) {
        document.getElementById("dropbox-chooser").appendChild(Dropbox.createChooseButton({
            success: function(files) {
                saveoperatorProfileExternalFile(files, 'dropbox');
            },
            linkType: "preview",
    ...
    /* operator profile tickets table */
    $('.table-tickets-single').find('#th-submitter').removeClass('toggleable');

    initDataTable('.table-tickets-single', admin_url + 'tickets/index/false/' + operator_id, undefined,
        undefined, 'undefined', [$('table thead .ticket_created_column').index(), 'desc']);

    /* operator profile contracts table */
    initDataTable('.table-contracts-single-operator', admin_url + 'contracts/table/' + operator_id, undefined,
    ...
    <?php } ?>
    _table_api = initDataTable('.table-contacts', admin_url + 'operators/contacts/' + operator_id,
    ...
    /* operator profile invoices table */
    initDataTable('.table-invoices-single-operator',
        admin_url + 'invoices/table/' + operator_id,
        'undefined',
        'undefined',
        'undefined', [
            [3, 'desc'],
    ...
    initDataTable('.table-credit-notes', admin_url + 'credit_notes/table/' + operator_id, ['undefined'], [
        'undefined'
    ], undefined, [0, 'desc']);

    /* operator profile Estimates table */
    initDataTable('.table-estimates-single-operator',
        admin_url + 'estimates/table/' + operator_id,
        'undefined',
        'undefined',
        'undefined', [
            [3, 'desc'],
    ...
    /* operator profile payments table */
    initDataTable('.table-payments-single-operator',
        admin_url + 'payments/table/' + operator_id, undefined, undefined,
        'undefined', [0, 'desc']);

    /* operator profile reminders table */
    initDataTable('.table-reminders', admin_url + 'misc/get_reminders/' + operator_id + '/' + 'operator',
        undefined, undefined, undefined, [1, 'asc']);

    /* operator profile expenses table */
    initDataTable('.table-expenses-single-operator',
        admin_url + 'expenses/table/' + operator_id,
        'undefined',
        'undefined',
        'undefined', [6, 'desc']);

    /* operator profile proposals table */
    initDataTable('.table-proposals-operator-profile',
        admin_url + 'proposals/proposal_relations/' + operator_id + '/operator',
        'undefined',
        'undefined',
        'undefined', [6, 'desc']);

    /* Custome profile projects table */
    initDataTable('.table-projects-single-operator', admin_url + 'projects/table/' + operator_id, undefined,
        undefined, 'undefined',
    ...
    appValidateForm($('.operator-form'), vRules);
    ...
    if (typeof(operator_id) == 'undefined') {
        $('#company').on('blur', function() {
            ...
            $.post(admin_url + 'operators/check_duplicate_operator_name', {
                    company: company
                })
    ...
    $('.billing-same-as-operator').on('click', function(e) {
        e.preventDefault();
    ...
    $('.operator-copy-billing-address').on('click', function(e) {
        e.preventDefault();
    ...
    $('.operator-form').on('submit', function() {
...
function delete_contact_profile_image(contact_id) {
    requestGet('operators/delete_contact_profile_image/' + contact_id).done(function() {
...
function operatorGoogleDriveSave(pickData) {
    saveoperatorProfileExternalFile(pickData, 'gdrive');
}

function saveoperatorProfileExternalFile(files, externalType) {
    $.post(admin_url + 'operators/add_external_attachment', {
        files: files,
...
function validate_contact_form() {
    appValidateForm('#contact-form', {
        ...
        email: {
            ...
            <?php } ?>
            email: true,
            // Use this hook only if the contacts are not logging into the operators area and you are not using support tickets piping.
...
function contact(operator_id, contact_id) {
    if (typeof(contact_id) == 'undefined') {
        contact_id = '';
    }
    requestGet('operators/form_contact/' + operator_id + '/' + contact_id).done(function(response) {
...
function update_all_proposal_emails_linked_to_contact(contact_id) {
    ...
    $.post(admin_url + 'operators/update_all_proposal_emails_linked_to_operator/' + contact_id, data).done(function(
        response) {
        response = JSON.parse(response);
        if (response.success) {
            alert_float('success', response.message);
...
function do_share_file_contacts(edit_contacts, file_id) {
    ...
    if (typeof(edit_contacts) == 'undefined' && typeof(file_id) == 'undefined') {
        ...
        $('#operator_file_share_file_with').modal('show');
        return;
    }
    var file_id = $('input[name="file_id"]').val();
    $.post(admin_url + 'operators/update_file_share_visibility', {
        file_id: file_id,
        share_contacts_id: contacts_shared_ids_selected,
        operator_id: $('input[name="userid"]').val()
    }).done(function() {
        window.location.reload();
    });
}

function save_longitude_and_latitude(operatorid) {
    var data = {};
    data.latitude = $('#latitude').val();
    data.longitude = $('#longitude').val();
    $.post(admin_url + 'operators/save_longitude_and_latitude/' + operatorid, data).done(function(response) {
        if (response == 'success') {
            alert_float('success', "<?php echo e(_l('updated_successfully', _l('operator'))); ?>");
        }
        setTimeout(function() {
            window.location.reload();
        }, 1200);
