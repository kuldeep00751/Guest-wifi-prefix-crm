...
<div class="modal fade" id="operator_file_share_file_with"
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <?= form_hidden('file_id'); ?>
                <?= render_select('share_contacts_id[]', $contacts, ['id', ['firstname', 'lastname']], 'operator_contacts', [get_primary_contact_user_id($operator->userid)], ['multiple' => true, 'data-actions-box' => true], [], '', '', false); ?>
            </div>
...
<h4 class="operator-profile-group-heading">
    <?= _l('operator_attachments'); ?>
</h4>
<p class="text-info">
    <?= _l('operator_files_info_message'); ?>
</p>
<?php if (isset($operator)) { ?>
<?= form_open_multipart(admin_url('operators/upload_attachment/' . $operator->userid), ['class' => 'dropzone', 'id' => 'operator-attachments-upload']); ?>
<input type="file" name="file" multiple />
<?= form_close(); ?>
<div class="tw-flex tw-justify-end tw-items-center tw-space-x-2 mtop15">
    <button class="gpicker" data-on-pick="operatorGoogleDriveSave">
    ...
    <div class="mtop25">
        ...
        <table class="table dt-table" data-order-col="2" data-order-type="desc">
            <thead>
                <tr>
                    <th width="30%">
                        <?= _l('operator_attachments_file'); ?>
                    </th>
                    <th><?= _l('operator_attachments_show_in_operators_area'); ?>
                    </th>
                ...
                <tr
                    ...
                    ?>
                        <a href="<?= $is_image ? ($lightBoxUrl ?: $img_url) : $attachment_url; ?>"
                            <?= $is_image ? 'data-lightbox="operator-profile"' : ''; ?>
                    ...
                    <td>
                        <div class="onoffswitch" <?php if ($type != 'operator') {?>
                            data-toggle="tooltip"
                            data-title="<?= _l('operator_attachments_show_notice'); ?>"
                            <?php } ?>>
                            <input type="checkbox"
                                <?= $type != 'operator' ? 'disabled' : ''; ?>
                            id="<?= e($_att['id']); ?>"
                            data-id="<?= e($_att['id']); ?>"
                            class="onoffswitch-checkbox operator_file"
                            data-switch-url="<?= admin_url(); ?>misc/toggle_file_visibility"
                            <?= isset($_att['visible_to_operator']) && $_att['visible_to_operator'] == 1 ? 'checked' : ''; ?>>
                            <label class="onoffswitch-label"
                                for="<?= e($_att['id']); ?>"></label>
                        </div>
                        <?php if ($type == 'operator' && $_att['visible_to_operator'] == 1) {
                            ...
                            echo '<span class="text-warning' . (empty($file_visibility_message) || total_rows(db_prefix() . 'contacts', ['userid' => $operator->userid]) == 0 ? ' hide' : '') . '">' . $file_visibility_message . '</span>';
                    ...
                    <td>
                        <div class="tw-flex tw-items-center tw-space-x-2">
                            ...
                            <a href="<?= admin_url('operators/delete_attachment/' . $_att['rel_id'] . '/' . $_att['id']); ?>"
...
<?php
$this->load->view('admin/operators/modals/send_file_modal');
} ?>