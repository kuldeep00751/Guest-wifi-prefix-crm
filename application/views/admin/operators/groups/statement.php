...
<h4 class="operator-profile-group-heading">
    <?= e(_l('operator_statement_for', get_company_name($operator->userid))); ?>
</h4>
<div class="row">
    <div class="col-md-4">
        <?php $this->load->view('admin/operators/groups/_statement_period_select', ['onChange' => 'render_operator_statement()']); ?>
    </div>
    <div class="col-md-8 col-xs-12">
        <div class="text-right _buttons pull-right tw-space-x-1">
            ...
            <a href="#" class="btn btn-default btn-with-tooltip sm:!tw-px-3" data-toggle="modal"
                data-target="#statement_send_to_operator"><span data-toggle="tooltip"
        ...
        <div class="row">
            ...
            <div class="col-md-7">
                <address>
                    ...
                    <?= format_operator_info($operator, 'statement', 'billing'); ?>
                </address>
            </div>
...
<div class="modal fade email-template"
    data-editor-id=".<?= 'tinymce-' . $operator->userid; ?>"
    id="statement_send_to_operator" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
...
if (count($selected) == 0) {
    echo '<p class="text-danger">' . _l('sending_email_contact_permissions_warning', _l('operator_permission_invoice')) . '</p><hr />';
}
...
?>
                        ...
                        <hr />
                        <h5 class="bold">
                            <?= _l('invoice_send_to_operator_preview_template'); ?>
                        </h5>
                        <hr />
                        <?= render_textarea('email_template_custom', '', $template->message, [], [], '', 'tinymce-' . $operator->userid); ?>
...
<?php hooks()->add_action('app_admin_footer', 'parse_operator_statement_html');
function parse_operator_statement_html()
{ ?>
<script>
    $(function() {
        render_operator_statement();
    });

    function render_operator_statement() {
        ...
        var statementUrl = admin_url + 'operators/statement';
        var statementUrlParams = new Array();

        statementUrlParams['operator_id'] = operator_id;
        ...
        $.get(statementUrl, function(response) {
            ...
            $('#statement_pdf').attr('href', buildUrl(admin_url + 'operators/statement_pdf', statementUrlParams));
            $('#send_statement_form').attr('action', buildUrl(admin_url + 'operators/send_statement',
                statementUrlParams));

            statementUrlParams['print'] = true;
            $('#statement_print').attr('href', buildUrl(admin_url + 'operators/statement_pdf',
                statementUrlParams));
