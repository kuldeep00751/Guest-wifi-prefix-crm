...
<a href="<?= admin_url('projects/project?operator_id=' . $operator->userid); ?>"
...
<dl class="tw-grid tw-gap-2 sm:tw-grid-flow-col sm:tw-auto-cols-max tw-mb-5 tw-overflow-x-auto">
    ...
    <div
        ...
        <span class="tw-font-semibold tw-mr-1 rtl:tw-ml-1">
            <?php $where = ($_where == '' ? '' : $_where . ' AND ') . 'status = ' . $status['id'] . ' AND operatorid=' . $operator->userid; ?>
...
<?php
       $this->load->view('admin/projects/table_html', ['class' => 'projects-single-operator']);
}
?>