...
class OperatorProfileBadges
{
    ...
    public function getBadge($feature)
    {
        ...
        $hook = hooks()->apply_filters('operators_profile_tab_badge', [
            'feature'     => $feature,
    ...
    /**
     * Get the total contacts for the operator profile badge
     *
     * @return int
     */
    public function contacts()
    {
        $this->CI->db->where('userid', $this->operatorId);
        $this->CI->db->where('usertype', 'operators');
        return $this->CI->db->count_all_results('contacts');
    }

    /**
     * Get the total notes for the operator profile badge
     *
     * @return int
     */
    public function notes()
    {
        $this->CI->db->where('rel_type', 'operator');
        $this->CI->db->where('rel_id', $this->operatorId);
    ...
    public function invoices()
    {
        ...
        $this->CI->db->where('clientid', $this->operatorId);
    ...
    public function contracts()
    {
        ...
        $this->CI->db->where('client', $this->operatorId);
    ...
    {
        $this->CI->db->where('operator_id', $this->operatorId);
    ...
    {
        $where = '(';
        $where .= '(rel_id IN (SELECT id FROM ' . db_prefix() . 'invoices WHERE clientid=' . $this->operatorId . ') AND rel_type="invoice")';
        $where .= ' OR (rel_id IN (SELECT id FROM ' . db_prefix() . 'estimates WHERE clientid=' . $this->operatorId . ') AND rel_type="estimate")';
        $where .= ' OR (rel_id IN (SELECT id FROM ' . db_prefix() . 'contracts WHERE client=' . $this->operatorId . ') AND rel_type="contract")';
        $where .= ' OR (rel_id IN (SELECT ticketid FROM ' . db_prefix() . 'tickets WHERE userid=' . $this->operatorId . ') AND rel_type="ticket")';
        $where .= ' OR (rel_id IN (SELECT id FROM ' . db_prefix() . 'expenses WHERE clientid=' . $this->operatorId . ') AND rel_type="expense")';
        $where .= ' OR (rel_id IN (SELECT id FROM ' . db_prefix() . 'proposals WHERE rel_id=' . $this->operatorId . ' AND rel_type="proposal") AND rel_type="proposal")';
        $where .= ' OR (rel_id IN (SELECT userid FROM ' . db_prefix() . 'operators WHERE userid=' . $this->operatorId . ') AND rel_type="operator")';
        $where .= ' OR (rel_id IN (SELECT id FROM ' . db_prefix() . 'projects WHERE userid=' . $this->operatorId . ') AND rel_type="project")';
        $where .= ')';
